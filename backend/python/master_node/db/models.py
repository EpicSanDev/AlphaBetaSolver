...
class ComputeNode(Base):
    ...
    node_metadata = Column(JSON)  # Renamed from 'metadata' which is reserved in SQLAlchemy
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class SimulationResult(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    simulation_id = Column(UUID(as_uuid=True), ForeignKey('simulations.id'), nullable=False)
    result_type = Column(String(50), nullable=False)  # 'strategy', 'exploitability', 'tree'
