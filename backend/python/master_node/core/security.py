...
class APIKeyAuth:
    ...
    def __init__(self):
        self.valid_keys = (
            settings.API_KEYS.split(",") if hasattr(settings, "API_KEYS") else []
        )
...
rate_limiter_simulation = RateLimiter(
    calls=10, period=60
)  # 10 simulation requests per minute
rate_limiter_status = RateLimiter(
    calls=200, period=60
)  # 200 status requests per minute
