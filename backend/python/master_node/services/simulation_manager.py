...
class SimulationManager:
    ...
    def __init__(self, task_queue: TaskQueue = None, storage_path: str = "/tmp/gto_simulations"):
    ...
    async def stop(self):
        ...
        if self.running_simulations:
            await asyncio.gather(*self.running_simulations.values(), return_exceptions=True)
    ...
    def _estimate_completion_time(self, simulation_type: str, solver_config: Dict[str, Any]) -> datetime:
    ...
    async def _run_simulation(self, simulation: Simulation):
        ...
        try:
            ...
            logger.error(f"Erreur lors de l'exécution de la simulation {simulation.simulation_id}: {e}")
    ...
    async def _decompose_simulation(self, simulation: Simulation) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Simulation {simulation.simulation_id} décomposée en {len(tasks)} tâches")
        return tasks
    ...
    async def _monitor_simulation_progress(self, simulation: Simulation):
        ...
        while completed_tasks < total_tasks:
            ...
            # Calculer le progrès
            completed_tasks = await self._count_completed_tasks(simulation.simulation_id)
            ...
            if completed_tasks > 0:
                ...
                simulation.estimated_time_remaining = int(estimated_total_time - elapsed_time)
    ...
    async def _process_task_result(self, simulation_id: str, result_data: Dict[str, Any]):
        ...
        for result_file in result_files:
            async with aiofiles.open(result_file, 'r') as f:
                ...
                if task_result.get("success") and task_result.get("result_data"):
                    ...
                    final_exploitability = max(final_exploitability, data.get("exploitability", 0.0))
    ...
    async def cancel_simulation(self, simulation_id: str) -> bool:
        ...
        simulation = self.simulations.get(simulation_id)
        if not simulation or simulation.status not in [SimulationStatus.PENDING, SimulationStatus.RUNNING]:
            return False
    ...
    async def _save_simulation(self, simulation: Simulation):
        ...
        if simulation.estimated_completion_time:
            data["estimated_completion_time"] = simulation.estimated_completion_time.isoformat()
